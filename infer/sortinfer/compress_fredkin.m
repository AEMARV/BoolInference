function [ bin] = compress_fredkin( bin,method)
%CONDITIONLINEAR Summary of this function goes here
...
for epoch = 1 : epochnum
    ...
    for iters = 1 :maxiters
        bitmask = and(bin(:,1),false);
        bitmask(:) = true;
        [bin,depth] = sortinfer(bin,or(bin(1,:),true),bitmask,newtrainlog,0);
        bin = reshape(bin,bitnum,[]);
        bin = bin(sortinv,:);
        %% verbose
