function [ output_args ] = gxor_hyb( input_args )
%GXOR_HYB Summary of this function goes here
%   Detailed explanatiobitmask = true(size(bin(:,1)));
...
bitmask(probbitwise==0 | probbitwise==1) = false;
 ...
 while(true)
    ...
    if(any(prob>0.5))
         bin(bitmask,mask) = xorh(prob>0.5, bin(bitmask,mask));
        return;
    else
